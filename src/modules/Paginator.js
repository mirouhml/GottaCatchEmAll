export default class Paginator {
  ...
  init() {
    ...
    if (this.numberOfPages > 1) {
      ...
      document.getElementById('paginator').onclick = (e) => {
        const diff = this.currentPage - this.maxPages;
        if (e.target.id === 'previous' || e.target.textContent === '«') {
          if ((diff) <= 0) { this.currentPage = 0; } else (this.currentPage = diff);
          this.buildPagination();
        } else if ((e.target.id === 'next' || e.target.textContent === '»')) {
          if (this.currentPage < this.numberOfPages - 2) {
            // eslint-disable-next-line max-len
            if ((diff) >= this.numberOfPages) { this.currentPage = this.numberOfPages; } else (this.currentPage += this.maxPages);
            this.buildPagination();
          }
        } else {
  ...
  buildPagination = () => {
    ...
    if (max >= this.numberOfPages) { max = this.numberOfPages; }
    for (let i = this.currentPage; i < max; i += 1) {
      const element = document.createElement('li');
      element.className = 'page-item';
      if (i < 9) { element.innerHTML = `<a class="page-link" href="#">0${i + 1}</a>`; } else { element.innerHTML = `<a class="page-link" href="#">${i + 1}</a>`; }
